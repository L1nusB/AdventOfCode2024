...
def traverse(top_map: np.ndarray, cur_pos: Tuple[int, int], cur_height: int, visited: Set[Tuple[int, int]], target_height: int = 9) -> Tuple[int, Set[Tuple[int, int]]]:
    """Uses stateful DFS (depth-first search) which keeps track of all visited fields to avoid loops and revisiting"""
    ...
    next_steps = [(i,j) for i,j in next_steps if ((i>=0 and i<top_map.shape[0]) and (j>=0 and j<top_map.shape[1]))]
    valid_heads = 0
    for step in next_steps:
        if top_map[*step] == cur_height+1:
            valid_sub_heads, visited = traverse(top_map, step, cur_height+1, visited, target_height)
...
if __name__ == "__main__":
    ...
    trailhead_values = [traverse(top_map, tuple(start), 0, set(), 9)[0] for start in trailheads]
